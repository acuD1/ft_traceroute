...
/*   Updated: 2021/06/22 15:06:11 by arsciand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint8_t  get_opts_args_handler(t_core *core, int argc, char **argv)
{
    ...
    {
        if (tmp_opt || (tmp_opt = get_opt_set_db(&core->opts_args->opt_set, "m")))
        {
            if (tmp_opt->arg)
            {
    ...
    {
        if (tmp_opt || (tmp_opt = get_opt_set_db(&core->opts_args->opt_set, "q")))
        {
            if (tmp_opt->arg)
            {
                if (ft_is_number(tmp_opt->arg))
